...
from text2mesh.render import Renderer
from text2mesh.mesh import Mesh
from text2mesh.Normalization import MeshNormalizer
import numpy as np
import random
import copy
import torchvision
...
class Rendering_CLIP:
    ...
    def rendering(self, V, name = None, step = None):
        ...
        sampled_mesh = mesh
        '''
        if name == 'normal' or name == 'z_ref_normal' or name == 'flame_code_sample_normal':
            self.default_color[:, :] = torch.tensor([0.5, 0.5, 0.5]).to(self.device)
        elif name == 'stylized' or name == 'flame_code_ref_stylized' or name == 'flam_code_sample_stylized':
            self.default_color[:, :] = torch.tensor([0.7, 0.0, 0.0]).to(self.device)
        else: 
            self.default_color[:, :] = torch.tensor([0.5, 0.5, 0.5]).to(self.device)
