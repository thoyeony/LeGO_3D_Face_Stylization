...
class Mesh():
    def __init__(self,obj_path = None, V = None, F = None, color=torch.tensor([0.0,0.0,1.0])):
        ...
        if V is not None and F is not None:
            ...
            #mesh = trimesh.Trimesh(vertices=V.detach().cpu().numpy(), faces=F.detach().cpu().numpy())
            self.vertices = V
            self.faces = F.to(device)
            self.vertex_normals = None
            self.face_normals = None
            self.texture_map = None
            self.face_uvs = None
