...
def run_branched(args):
    ...
    mesh_normal = Mesh(args.obj_path)
    MeshNormalizer(mesh_normal)()
    mesh_stylized = Mesh('./flame_mesh/stylized.obj')
    MeshNormalizer(mesh_stylized)()

    prior_color = torch.full(size=(mesh_normal.faces.shape[0], 3, 3), fill_value=0.5, device=device)
    ...
    clip_normalizer = transforms.Normalize((0.48145466, 0.4578275, 0.40821073), (0.26862954, 0.26130258, 0.27577711))#((mean_R, mean_G, mean_B), (sd_R, sd_G, sd_B)
    # CLIP Transform
    ...
    loss_check = None
    vertices_normal = copy.deepcopy(mesh_normal.vertices)
    vertices_stylized = copy.deepcopy(mesh_stylized.vertices)
    '''
    network_input = copy.deepcopy(vertices_normal)
    ...
    '''
    #==================Cosine similarity of Clip embedded uncolored meshes======================
    sampled_mesh_normal = mesh_normal
    sampled_mesh_stylized = mesh_stylized 
    
    if args.geoloss:
            default_color = torch.zeros(len(mesh_normal.vertices), 3).to(device)
            default_color[:, :] = torch.tensor([0.5, 0.5, 0.5]).to(device)
            #Normal face mesh redering 
            sampled_mesh_normal.face_attributes = kaolin.ops.mesh.index_vertices_by_faces(default_color.unsqueeze(0),
                                                                                   sampled_mesh_normal.faces)
            geo_renders_normal, elev_normal, azim_normal = render.render_front_views(sampled_mesh_normal, num_views=args.n_views,
                                                                show=args.show,
            ...
            #Stylized face mesh redering 
            sampled_mesh_stylized.face_attributes = kaolin.ops.mesh.index_vertices_by_faces(default_color.unsqueeze(0),
                                                                                   sampled_mesh_stylized.faces)
            geo_renders_stylized, elev_stylized, azim_stylized = render.render_front_views(sampled_mesh_stylized, num_views=args.n_views,
                                                                show=args.show,
                                                                ...
                                                                return_views=True,
                                                                background=background)
            if not os.path.exists(os.path.join(dir, 'normal')): 
                os.mkdir(os.path.join(dir, 'normal'))
                
            if not os.path.exists(os.path.join(dir, 'stylized')): 
                os.mkdir(os.path.join(dir, 'stylized'))
                
            for i, img in enumerate(geo_renders_normal): 
                torchvision.utils.save_image(img, os.path.join(os.path.join(dir, 'normal'), 'view5_{}.jpg'.format(i)))
            
            for i, img in enumerate(geo_renders_stylized): 
                torchvision.utils.save_image(img, os.path.join(os.path.join(dir, 'stylized'), 'view5_{}.jpg'.format(i)))
            
            if args.n_normaugs > 0:
                accumulated_score = 0
                normloss = 0.0
                ### avgview != aug
                for idx in range(args.n_normaugs):
                    augmented_image_normal = displaugment_transform(geo_renders_normal)
                    augmented_image_stylized = displaugment_transform(geo_renders_stylized)
                    
                    for i, img in enumerate(augmented_image_normal): 
                        torchvision.utils.save_image(img, os.path.join(os.path.join(dir, 'normal'), 'aug4_{}_{}.jpg'.format(idx,i)))
                    
                    for i, img in enumerate(augmented_image_stylized): 
                        torchvision.utils.save_image(img, os.path.join(os.path.join(dir, 'stylized'), 'aug4_{}_{}.jpg'.format(idx,i)))
                    
                    encoded_mesh_normal = clip_model.encode_image(augmented_image_normal)
                    encoded_mesh_stylized = clip_model.encode_image(augmented_image_stylized)
                    
                    score = torch.cosine_similarity(torch.mean(encoded_mesh_normal, dim=0),
                                                            torch.mean(encoded_mesh_stylized, dim=0), dim=0)
                    accumulated_score += score
                    print('Score: ',score) 
                
                print('Accum_score: ', accumulated_score)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--no_prompt', default=True, action='store_true')
